...
#endif
Vector3 BSP::CalcCenterPositionFromLeafList(const std::vector<StEntityPtr>& leafArray)
{
    // まずは、AABBの中心座標を求める。
    Vector3 centerPos;
    for (const auto& leafPtr : leafArray) {
        auto leaf = static_cast<StLeaf*>(leafPtr.get());
...
void BSP::CalcCovarianceMatrixFromLeafNodeList(
    ...
    const std::vector<StEntityPtr>& leafNodeArray,
...
)
{
    ...
    if (plane.normal.Length() < 0.1f) {
        ...
        StLeaf* leafFront = static_cast<StLeaf*>(leafArray.front().get());
        StLeaf* leafBack = static_cast<StLeaf*>(leafArray.back().get());
...
BSP::StEntityPtr BSP::CreateBSPTreeEntity_LeafList(const std::vector<StEntityPtr>& leafArray)
{
    auto newNodePtr = std::make_shared<StLeafList>();
    auto leafList = static_cast<StLeafList*>(newNodePtr.get());
    leafList->type = enEntityType_LeafList;
    leafList->leafList = leafArray;
    return newNodePtr;
}
BSP::StEntityPtr BSP::CreateBSPTreeEntity(const std::vector<StEntityPtr>& leafArray)
{
    if (leafArray.size() == 1) {
        // リーフエンティティを返す。
...
void BSP::WalkTree(StEntityPtr entityPtr, const Vector3& pos, std::function<void(StLeaf* leaf)> onEndWalk) const
{
    if (entityPtr->type == enEntityType_Node) {
        // これはノードなのでさらに潜る。
        // 左に潜る？右に潜る？
        StNode* node = static_cast<StNode*>(entityPtr.get());
        for (StEntityPtr& entity : node->leafArray) {
        ...
        // リーフに到達した。
        onEndWalk(static_cast<StLeaf*>(entityPtr.get()));
    }
    else if (entityPtr->type == enEntityType_LeafList) {
        // リーフのリストに到達した。
        auto leafList = static_cast<StLeafList*>(entityPtr.get());
        for (auto leaf : leafList->leafList) {
            onEndWalk(static_cast<StLeaf*>(leaf.get()));
        }
    }
}
void BSP::WalkTree(const Vector3& pos, std::function<void(StLeaf* leaf)> onEndWalk) const
{
